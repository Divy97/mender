...
description: >-
  ...
  classification. Invoke: /raftkit-bug <asana-link-or-gid>, /raftkit-bug <url>,
  /raftkit-bug <description>, /raftkit-bug (interactive)
---
...
This skill drives a structured workflow for resolving **any** bug — whether reported by QA in Asana, discovered during development, described by a user, or surfaced by an error trace. Unlike `/raftkit-dev` which starts from pre-written task specs, `/raftkit-bug` starts from a problem and works backward: understand the issue, classify its domain, investigate root cause, analyze negative scenarios, propose a fix, and only then implement.
...
Parse the `/raftkit-bug` invocation to determine the entry type and extract relevant context.

### Entry Type Detection (evaluated in order)

| Entry Type                    | Detection                                                                                                                        | Example                                                                                        |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **Asana task**                | URL contains `app.asana.com`, OR raw numeric string with 10+ digits                                                              | `/raftkit-bug https://app.asana.com/0/0/1234567890/f` or `/raftkit-bug 1234567890`             |
| **URL to inspect**            | Starts with `http://` or `https://` but is NOT an Asana URL                                                                      | `/raftkit-bug https://localhost:4001/courses`                                                  |
| **Error trace**               | Contains stack-trace markers: `Error:`, `at `, `.ts:`, `.tsx:`, `Traceback`, or multiline content with file paths + line numbers | `/raftkit-bug TypeError: Cannot read properties of undefined (reading 'map') at CourseList...` |
| **Free-text description**     | Any other non-empty argument                                                                                                     | `/raftkit-bug the signup form doesn't show validation errors`                                  |
| **Interactive (no argument)** | Empty `/raftkit-bug` invocation                                                                                                  | `/raftkit-bug`                                                                                 |
...
1. **Invoke `/raftkit-asana` skill** — MANDATORY before any Asana MCP call (per project memory)
...
| -------------------------------------------------------------------------------------------------------- | -------------- | --------------------------------------------------------------------------------- |
| Error in `packages/api/`, `packages/auth/`, `packages/validators/`, `packages/queue/`, `packages/email/` | `backend`      | `/raftkit-backend`                                                                |
| Error in `packages/db/`, constraint violations, migration issues, query errors                           | `database`     | `/raftkit-database`                                                               |
| Error in `apps/web-*/`, `packages/ui/`, React components, CSS/Tailwind, hydration                        | `frontend`     | `/raftkit-react`, `/next-best-practices`                                          |
| Error spans both API + web app layers                                                                    | `fullstack`    | `/raftkit-backend`, `/raftkit-database`, `/raftkit-react`, `/next-best-practices` |
| Error in `apps/mobile/`, `packages/ui-mobile/`                                                           | `mobile`       | (no Playwright, no Next.js tools)                                                 |
| Auth-related: permissions, roles, sessions, tokens, Better-Auth                                          | `auth`         | `/raftkit-backend` (auth focus)                                                   |
...
1. **Invoke `/raftkit-code-quality`** — ensure proposed changes follow quality standards (30-line function limit, no magic numbers, DRY, proper naming)
...
2. **If user says yes:**
   ...
   **8c. Implement with TDD** — invoke `/raftkit-tdd` skill (project-specific):
   ...
   **8d. Code quality** — invoke `/raftkit-code-quality` skill:
...
3. **If user says no:**
   - End gracefully: "Investigation documented. <If Asana: 'The Asana comment has been posted with findings.'> You can re-invoke `/raftkit-bug` later to pick up implementation."
...
| Phase | Skill                                        | When                                              |
| ----- | -------------------------------------------- | ------------------------------------------------- |
| 1     | `/raftkit-asana`                             | Asana entry — MANDATORY before any Asana MCP call |
| 1     | Playwright MCP                               | URL entry — inspect the page                      |
| 2     | `/raftkit-backend`, `/raftkit-database`      | Backend/database classification                   |
| 2     | `/raftkit-react`, `/next-best-practices`     | Frontend classification                           |
...
| 6     | `/raftkit-code-quality`                      | Always — validate proposed changes                |
...
| 8     | `/raftkit-tdd`                               | During implementation — project-specific TDD      |
| 8     | `/raftkit-code-quality`                      | During implementation — quality enforcement       |
...
| Skill            | When                           | Why                                      |
| ---------------- | ------------------------------ | ---------------------------------------- |
| `/raftkit-asana` | Phase 1a (before any MCP call) | MANDATORY — loads Asana MCP context      |
| `/docx`          | Phase 3 (conditional)          | Read Proposal Document.docx              |
| `/raftkit-dev`   | Phase 8 (optional)             | If the fix maps to an existing task spec |
...
<HARD-GATE>
NEVER invoke Asana MCP tools without first invoking the /raftkit-asana skill.
</HARD-GATE>
