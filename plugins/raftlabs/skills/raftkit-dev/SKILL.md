...
description: >-
  ...
  /raftkit-dev @path/to/task.md [instructions],
  /raftkit-dev @path/to/milestone/ [instructions]
---
...
Parse the `/raftkit-dev` invocation to determine mode and extract arguments.

### Invocation Patterns

| Pattern                   | Example                                                                            | Mode      |
| ------------------------- | ---------------------------------------------------------------------------------- | --------- |
| Single task               | `/raftkit-dev @docs/project-management/05-courses/course-api.md`                   | single    |
| Single task + instruct.   | `/raftkit-dev @docs/project-management/05-courses/course-api.md Skip auth for now` | single    |
| Milestone directory       | `/raftkit-dev @docs/project-management/05-courses/`                                | milestone |
| Milestone dir + instruct. | `/raftkit-dev @docs/project-management/05-courses/ Focus on API first`             | milestone |
...
3. **Milestone task queue** — in milestone mode:
   ...
   - Once confirmed, the workflow runs continuously through all remaining tasks — each task gets the full Phase 1-7b treatment (research, plan, branch, TDD, commits, verify, PR). The user does not need to re-invoke `/raftkit-dev` between tasks.
...
The `/raftkit-dev` skill automatically enters plan mode for Phases 1-2 (research-only phases where no code is written).

### Protocol

1. **On `/raftkit-dev` invocation**, check if currently in plan mode:
...
4. **Fallback** — if `EnterPlanMode` tool is unavailable or errors, instruct the user: "Please type `/plan` to enter plan mode, then re-invoke `/raftkit-dev`"
  ...
  "negativeScenarios": [
    {
      ...
      "relevant": true,
      "handled": "done",
      "finding": "Empty state added for course list",
      "testFile": "packages/api/src/routers/__tests__/courses.test.ts:145"
    }
  ]
}
```
...
- `phase4b` added — tracks Negative Scenario Analysis phase status (`"pending"`, `"in_progress"`, `"completed"`)
...
- `negativeScenarios` array added — persists Phase 4b findings for Phase 6 re-check. Each entry: `category`, `relevant`, `handled` (`"done"`, `"partial"`, `"no"`, `"n/a"`), `finding`, `testFile`
...
1. **On `/raftkit-dev` invocation** → check for existing session file at `docs/plans/.dev-session.json`:
   - **Exists** → **Resume flow:**
...
6. **Milestone transitions** — in milestone mode, after Phase 7b for a task:
   ...
   - Re-enter plan mode for the next task, then **immediately begin Phase 1 for that task** — do NOT stop or wait for a new `/raftkit-dev` invocation. The milestone workflow is a continuous pipeline.
...
The Ralph Loop plugin (`ralph-loop`) creates a self-referential iteration loop where `/raftkit-dev` is re-invoked with the same prompt until all acceptance criteria are met. When active, `/raftkit-dev` must adapt its behavior for idempotency, auto-proceed, and completion signaling.

### Detection

On every `/raftkit-dev` invocation, check for the Ralph Loop state file:
...
| Phase 4b  | Re-run if Phase 4 produced new code this iteration; skip if `phase4b: "completed"` AND no new code since last run |
...
2. **Design Interview** — walk through every aspect of the brainstormed design and resolve each branch of the decision tree before writing a plan. The goal is shared understanding — no ambiguity should survive this step.

   **How it works:**
   - Identify every open decision, dependency, and assumption from the brainstorm output
   - Ask questions **one at a time**, providing your recommended answer for each
   - Before asking the user, check whether the question can be answered by:
     - **Exploring the codebase** — read relevant files, grep for patterns, check existing implementations
     - **Research Protocol** — query Context7, specialized MCPs, or WebSearch (see Research Hierarchy)
     - If the answer is found, state the finding and your conclusion instead of asking. Only escalate to the user when the answer requires a judgment call or preference
   - After each answer (yours or the user's), check if it creates downstream decisions — if so, follow that branch immediately before moving on
   - Continue until all branches are resolved. A decision tree is resolved when every leaf has a concrete, actionable answer — no "we'll figure it out later"

   **What to cover** (non-exhaustive — adapt to the task):
   - Data model choices (schema shape, relations, indexes, constraints)
   - API design (endpoint shape, input/output contracts, error responses)
   - Component boundaries (what's shared vs. app-specific, new files vs. existing)
   - State management (server state, client state, cache strategy)
   - Auth/permission model (who can do what, where is it enforced)
   - Edge cases the spec doesn't address (empty states, concurrent access, error recovery)
   - Migration path (if changing existing behavior — backwards compatibility, data migration)
   - Anything from the brainstorm that had multiple viable options

   **Exit condition:** All decisions resolved. Summarize the resolved decisions in a compact list before proceeding.

3. **Research all technical decisions** — extract every library, framework, and API from the resolved design, then verify each one:
...
4. **After research and design approval, invoke `superpowers:writing-plans`** — create a detailed implementation plan incorporating all resolved decisions from the Design Interview:
   ...
   - Include a **Resolved Decisions** section referencing key outcomes from step 2
...
5. **Save the plan** to `docs/plans/YYYY-MM-DD-<task-slug>-plan.md`
   - Use today's date and a slug derived from the task title
   - Include the task file path as a reference

6. **Get explicit user approval** before proceeding to Phase 3.
...
## Phase 4b: Negative Scenario Analysis

**Goal:** Proactively identify failure modes, edge cases, and unhandled states in the code just implemented. Any gaps become additional TDD cycles before committing.

<HARD-GATE>
NEVER skip this phase. Every task must evaluate negative scenarios against the implemented code.
Even if every acceptance criterion passes, the code may have unhandled failure modes that the spec did not anticipate.
</HARD-GATE>

### When It Runs

- **After** all acceptance criteria from Phase 4 have status `"done"` (or `"partial"` with user approval)
- **Before** the final commit batch in Phase 5

### Procedure

1. **Scan the implemented code** — read all files created or modified during Phase 4. Build a list of:
   - API procedures/endpoints touched
   - DB queries and mutations
   - UI components and pages
   - Validators and schemas
   - Auth/permission checks

2. **Evaluate the 6-category checklist** against the actual code:

#### 4b.1 Error States

- API call failures (500, 503, timeout) — is there an `error.tsx` boundary? Are oRPC errors caught on the client?
- DB query errors — are they handled or do they crash the handler?
- Are errors surfaced to the user or silently swallowed?

#### 4b.2 Empty/Null/Undefined States

- Empty arrays — does the UI render an empty state, or crash?
- Missing optional fields — are they guarded (`?.`, `?? default`)?
- Loading states — what renders before data arrives?
- First-time user experience — what if there are zero items?
- `noUncheckedIndexedAccess` compliance — are array index accesses guarded?

#### 4b.3 Boundary Conditions

- Max-length inputs — do Zod schemas enforce limits matching DB column constraints?
- Special characters (quotes, HTML entities, emoji) — are they escaped/handled?
- Minimum/zero/negative values — are they validated?
- Large datasets — is pagination present where needed?

#### 4b.4 Auth/Permission Scenarios

- Is the API procedure protected (`protectedProcedure`) or public?
- Are role checks present at the API level (not just UI hiding)?
- Does unauthenticated access redirect correctly?

#### 4b.5 Concurrency/Race Conditions

- Double-submit — is the submit button disabled during request (`isPending`)?
- Stale data — is TanStack Query cache invalidated after mutations?
- Optimistic updates — what if the server rejects?

#### 4b.6 Network Failures

- Client-side error handling — are oRPC errors caught and displayed?
- Loading/pending states — do they exist for async operations?
- Retry logic — is it needed for this feature?

3. **Output the analysis table:**
...
|---|----------|-----------|----------|---------|
| 1 | API failure | Yes | No | No error boundary on /courses/[id] page |
| 2 | Empty data | Yes | Partial | Empty state for list, but detail page crashes on null |
| 3 | Max length | Yes | No | Zod allows 500 chars, DB column is varchar(255) |
| 4 | Unauth access | Yes | Yes | protectedProcedure + redirect in place |
| 5 | Double submit | Yes | No | Submit button not disabled during mutation |
| 6 | Network error | Yes | Partial | Error caught but no user-visible message |
```

4. **Generate additional work items** — every row with "No" or "Partial" in the Handled column becomes a TDD cycle:
   - Write a failing test for the gap
   - Implement the fix
   - Verify the test passes

5. **Re-enter Phase 4 implementation loop** for each gap, following the same Red-Green-Refactor pattern.

6. **Update the table** — after fixing, re-assess each row. All rows should show "Yes" in Handled column before proceeding.

### Scope Calibration

Unlike `/raftkit-bug` (which investigates an unknown problem), `/raftkit-dev` starts from a spec. The analysis here should be:

- **Focused** on the code actually written/modified in Phase 4
- **Practical** — skip categories that genuinely do not apply (e.g., no text input means boundary conditions for text length are irrelevant)
- **Efficient** — a 5-minute analysis, not a 30-minute investigation
- Mark irrelevant categories as "N/A" with a one-line reason, do not force findings

### Session Update

After all gaps are addressed, update the session:

- Mark `phase4b: "completed"`
- Save findings to `negativeScenarios` array in the session for Phase 6 reference
...
### Negative Scenario Re-Check

In addition to acceptance criteria, verify that all negative scenario gaps from Phase 4b remain addressed:

```
| # | Category | Finding from 4b | Still Handled? | Evidence |
|---|----------|-----------------|----------------|----------|
| 1 | Empty data | Added empty state | Yes | EmptyState component renders at line 42 |
| 2 | Double submit | Disabled button | Yes | isPending passed to disabled prop |
```

If any gap has regressed (e.g., a refactor removed the guard), fix it before proceeding.
...
3. **PR body template:**
   ...
   - [x] Negative scenarios analyzed (error states, empty data, boundaries, auth, concurrency, network)
...
1. **Transition summary** — display a brief status:
   ...
   ───────────────────────────────────────────
   ```
2. **Auto-continue** — immediately proceed to Phase 1 (Task Review & Status Audit) for the next task file. Do NOT wait for user to re-invoke `/raftkit-dev`.
...
14. NEVER skip Negative Scenario Analysis (Phase 4b).
    Every task must evaluate error states, empty data, boundary conditions,
    auth, concurrency, and network failures against the implemented code.
```
...
Phase 3: Branch creation
Phase 4: TDD implementation
Phase 4b: Negative scenario analysis (error states, empty data, boundaries, auth, concurrency, network)
...
5. **Conversation context loss** → re-read session file on next `/raftkit-dev` invocation to restore full context.
...
| Skipping the Design Interview          | Unresolved decisions surface mid-impl  | Resolve every branch of the decision tree in Phase 2 step 2         |
| Asking all interview Qs to user        | Wastes time on answerable questions    | Explore codebase / Research Protocol first, only ask judgment calls |
...
| Skipping negative scenario analysis    | Obvious bugs slip through to PR        | Always run Phase 4b after TDD implementation                        |
